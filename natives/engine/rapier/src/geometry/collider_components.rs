...
pub struct ColliderParent {
    ...
    /// IPL atlas (image colliders): optional portal prefix `P`. When set, the
    /// collider's effective world pose is `P × bodyPose × pos_wrt_parent` — the
    /// collider is this body's image through a portal isometry, colliding in the
    /// far chart while its contacts act on the parent body. NOT applied to
    /// mass-properties composition (images are massless by construction) nor to
    /// the CCD paths (CCD stays disabled for imaged bodies).
    pub portal_prefix: Option<Pose>,
}

impl ColliderParent {
    /// The collider's world pose derived from its parent's pose, including the
    /// IPL portal prefix when present. Every non-CCD composition of
    /// `parent_pos * pos_wrt_parent` must go through this.
    #[inline]
    pub fn collider_pose(&self, parent_pos: &Pose) -> Pose {
        let unprefixed = *parent_pos * self.pos_wrt_parent;
        match &self.portal_prefix {
            Some(prefix) => *prefix * unprefixed,
            None => unprefixed,
        }
    }
}
